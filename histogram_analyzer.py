...
class HistogramAnalyzer:
    ...
    def analyze(self, image: torch.Tensor, width: int, height: int, bins: int, 
        ...
        tensors = []
        for pil_img in [rgb_image, individual_image, luminance_image, hue_image, hsv_image, all_types_grid]:
...
class HistogramComparator:
    ...
    def compare(self, image_1: torch.Tensor, image_2: torch.Tensor, width: int, height: int, 
        ...
        rgb_comp = self._create_comparison(
            self.histogram_gen.draw_histogram_rgb(hist1, half_width, height, show_grid, True, stats1),
            self.histogram_gen.draw_histogram_rgb(hist2, half_width, height, show_grid, True, stats2),
            width,
            "RGB Histogram Comparison"
        )
        
        individual_comp = self._create_comparison(
            self.histogram_gen.draw_histogram_individual(hist1, half_width, height, show_grid, True, stats1),
            self.histogram_gen.draw_histogram_individual(hist2, half_width, height, show_grid, True, stats2),
            width,
            "Individual Channel Comparison"
        )
        
        luminance_comp = self._create_comparison(
            self.histogram_gen.draw_histogram_luminance(hist1, half_width, height, show_grid, True, stats1),
            self.histogram_gen.draw_histogram_luminance(hist2, half_width, height, show_grid, True, stats2),
            width,
            "Luminance Histogram Comparison"
        )
        
        hue_comp = self._create_comparison(
            self.histogram_gen.draw_histogram_hue(hist1, half_width, height, show_grid, True, stats1, hist1.get('hsv')),
            self.histogram_gen.draw_histogram_hue(hist2, half_width, height, show_grid, True, stats2, hist2.get('hsv')),
            width,
            "Hue Distribution Comparison"
        )
        
        # Create all-types grid (4 rows x 1 column: rows=types)
        all_types_grid = self.histogram_gen.create_histogram_comparison_grid(
